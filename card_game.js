...
function buildDeck () {
  ...
  for (let i = 0; i < ranks.length; i++) {
    for (let j = 0; j < suits.length; j++) {
      deck.push({
        'suit': suits[j], 'rank': ranks[i], 'value': j
      });
    }
...
function guess(card1, card2) {
  ...
  switch (input) {
    ...
    case "l":
      return compare(card1, card2) > 0;
    default:
      console.log("You need to guess h or l. No points for this round.");
      return false;
  }
}
...
function playGame() {
  ...
  while (score < 5 && score < deck.length) {
    ...
    if (guess(currentCard, nextCard) == true) {
      score++;
      console.log(`Congratulations, you guess right! Your score is now ${score}\n`)
    }
    else {
      console.log(`Your guess was wrong, you get no points.\n`);
    }
    currentCard = nextCard;
  }
  (deck.length != 0) ? console.log(`${playerName} you won!`): console.log(`You are out of cards, you lose.`)
}
